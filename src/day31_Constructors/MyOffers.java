...
public class MyOffers {
    public static void main(String[] args) {
        ...
        offer4.setInfo("TX","CapitalOne","SM",115000,true,true,false,true);

        Offer offer5=new Offer();
        offer5.setInfo("WA","Bank Of America","QA",130000,true,true,true,true);

        Offer[] muOffer={offer1,offer2,offer3,offer4,offer5};


        ArrayList<Offer> fullTimeOffer =new ArrayList<>(Arrays.asList(muOffer));
        fullTimeOffer.removeIf(p-> !p.isFullTime);// remove if the offer not fullTime

        ArrayList<Offer> localOffers =new ArrayList<>(Arrays.asList(muOffer));
        localOffers.removeIf(p-> !p.location.equals("VA"));// removes the offer not from local area
